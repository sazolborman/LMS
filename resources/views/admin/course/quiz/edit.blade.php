@php
    $lesson_data = App\Models\Lesson::where('id', $lesson_id)->first();
    $duration = $lesson_data->duration ? explode(':', $lesson_data->duration) : [];
@endphp
<form action="{{ route('admin.quiz.update') }}" method="post">
    ...
    <div class="fpb7 mb-3">
        ...
        <input class="form-control ad-form-control" type="text" id="title" name="title"
            value="{{ $lesson_data->title }}" required>
    </div>
        ...
        <div class="col-sm-12 fpb-7">
            ...
            <select class="form-control ad-form-control ol-select2" data-toggle="select2" name="section">
                ...
                @foreach (App\Models\Section::where('course_id', $course_id)->get() as $section)
                    <option @selected($lesson_data->section_id == $section->id) value="{{ $section->id }}">{{ $section->title }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-sm-4">
            ...
            <input class="form-control ad-form-control" type="number" min="1" id="total_mark" name="total_mark"
                required value="{{ $lesson_data->total_mark }}">
        </div>
        <div class="col-sm-4">
            ...
            <input class="form-control ad-form-control" type="number" min="1" id="pass_mark" name="pass_mark"
                required value="{{ $lesson_data->pass_mark }}">
        </div>
        <div class="col-sm-4">
            ...
            <input class="form-control ad-form-control" type="number" min="1" id="retake" name="retake"
                value="1" required value="{{ $lesson_data->retake }}">
        </div>
    </div>

    <div class="fpb-7 mb-3">
        <label for="description" class="form-label ad-form-label col-form-label">{{ translate('Description') }}</label>
        <textarea name="description" rows="5" class="form-control ad-form-control text_editor">{{ $lesson_data->description }}</textarea>
    </div>
