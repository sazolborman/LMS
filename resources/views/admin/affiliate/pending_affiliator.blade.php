@if (count($pending_affiliators) > 0)
    <div class="table-responsive dashboard-table-responsive mb-2 mt-4">
        <table class="table dashboard-table dashboard-table-3">
            ...
            <tbody>
                @foreach ($pending_affiliators as $affiliator)
                    <tr>
                        <td>
...
@if (count($pending_affiliators) > 0)
    <div class="itemshow-pagination d-flex align-items-center justify-content-between">
        <p class="fs-12px">
            {{ translate('Showing') . ' ' . $pending_affiliators->firstItem() . ' ' . translate('to') . ' ' . count($pending_affiliators) . ' ' . translate('of') . ' ' . $pending_affiliators->total() . ' ' . translate('entries') }}
        </p>
        <!-- Pagination -->
        {{ $pending_affiliators->links('admin.pagination.index') }}
    </div>
@endif
