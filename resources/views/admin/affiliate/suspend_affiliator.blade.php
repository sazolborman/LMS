@if (count($suspend_affiliators) > 0)
    <div class="table-responsive dashboard-table-responsive mb-2 mt-4">
        <table class="table dashboard-table dashboard-table-3">
            ...
            <tbody>
                @foreach ($suspend_affiliators as $affiliator)
                    <tr>
                        <td>
...
@if (count($suspend_affiliators) > 0)
    <div class="itemshow-pagination d-flex align-items-center justify-content-between">
        <p class="fs-12px">
            {{ translate('Showing') . ' ' . $suspend_affiliators->firstItem() . ' ' . translate('to') . ' ' . count($suspend_affiliators) . ' ' . translate('of') . ' ' . $suspend_affiliators->total() . ' ' . translate('entries') }}
        </p>
        <!-- Pagination -->
        {{ $suspend_affiliators->links('admin.pagination.index') }}
    </div>
@endif
