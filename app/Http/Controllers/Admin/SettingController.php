...
class SettingController extends Controller
{
    ...
    {
        return view('admin::setting.system_setting.index');
    }
    ...
    public function update(Request $request)
    {
        ...
        $product_current_version = Setting::where('type', 'version')->value('description');

        $rules = array('file' => 'required|file|mimes:zip');
        $validator = Validator::make($request->all(), $rules);
        if ($validator->fails()) {
            return redirect()->back()->with('error', translate('Select a valid zip file'));
        }
        ...
        if (class_exists('ZipArchive')) {
            ...
            return redirect()->back()->with('error', translate('Your server is unable to extract the zip file') . '. ' . translate('Please enable the zip extension to the server, then try again'));
        }
        ...
        if (array_key_exists('is_addon', $config) && strval($config['is_addon']) == "1") {
            ...
            if ($config['product_version']['minimum_required_version'] > $product_current_version) {
                return redirect()->back()->with('error', translate("You have to update your main application's version.") . '(' . $config['product_version']['minimum_required_version'] . ') ' . translate(' to install the addon'));
            }
            ...
            if (strval($config['addon_version']['minimum_required_version']) != strval($addons_current_version)) {
                return redirect()->back()->with('error', translate('It looks like you are skipping a version') . '. ' . translate('Please update version') . ' ' . $config['addon_version']['minimum_required_version'] . ' ' . translate('first'));
            }
            ...
            if (strval($config['product_version']['minimum_required_version']) != strval($product_current_version)) {
                return redirect()->back()->with('error', translate('It looks like you are skipping a version') . '. ' . translate('Please update version') . ' ' . $config['product_version']['minimum_required_version'] . ' ' . translate('first'));
            }
        }
        ...
        if (array_key_exists('is_addon', $config) && strval($config['is_addon']) == "1") {
            if ($config['addon_version']['minimum_required_version'] == "0") {
                return redirect()->back()->with('success', translate('Addon installed successfully'));
            } else {
                return redirect()->back()->with('success', translate('Addon updated successfully'));
            }
        } else {
            DB::table('settings')->where('type', 'version')->update(['description' => $config['product_version']['update_version']]);
            return redirect()->back()->with('success', translate('Version updated successfully'));
        }
    }
    ...
    {
        return view('admin::setting.website_setting.index');
    }
}
